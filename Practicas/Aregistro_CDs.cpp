...
{	
	...
	{
		for (int i = 1; i < strlen(NombreArtista); i++)
		{		
			if(NombreArtista[0] >= 'A' && NombreArtista[0] <= 'Z' && NombreArtista[i] >= 'a' && NombreArtista[i] <= 'z' && !isdigit(NombreArtista[i]))
				return 1;
			else
				return 0;
		}
...
void Alta()
{
	...
	do
	{
		...
		if((pf = fopen(carpeta_ori,"ab+"))==NULL)
		{
			system("color C");
...
void Editar ()
{
	do
	{
		...
		system("cls");
		cout << "\nIntroduce El Nombre De Un Album Para Buscarlo Y Editarlo: "; fflush(stdin);
		gets(buscar);
		...
		do
		{
			...
			if(!strcmpi(buscar,dat.NombreAlbum) && !feof(pf) && dat.Reliminado == 0)
			{
		        ...
		        do
				{
					gotoxy(30,11); cout << "¿Este Es El Registro Que Quieres Modificar? [S] / [N]";
					respuesta=getch();
					
					if(respuesta!= 'n' && respuesta!= 'N' && respuesta!= 's' && respuesta!= 'S' )
					{
						gotoxy(47,13); cout << "Introduce Una Opcion: ";
						getch();
					}
		...
		{
			system("cls");
			gotoxy(42,14); cout << "¿Deseas Seguir Modificando? [S] / [N]";
			respuesta2=getch();
...
{	
	...
	{	
		...
		system("cls");
		cout << "\nIntroduce el Nombre de un Album para Buscarlo y Eliminarlo: "; fflush(stdin);
		gets(buscar);
		...
		do
		{
			...
			if(!strcmpi(buscar,dat.NombreAlbum) && !feof(pf) && dat.Reliminado == 0)
			{
				...
				do
				{
					gotoxy(30,11); cout << "¿Este es el Registro que Quieres Eliminar [S] / [N]";
					respuesta=getch();
		...
		{
			system("cls");
			gotoxy(42,14); cout << "¿Deseas Seguir Eliminando? [S] / [N]";
			respuesta2=getch();
...
{	
	...
	{	
		...
		system("cls");
		cout << "\nIntroduce el Nombre del Album para Buscarlo y Restaurarlo: "; fflush(stdin);
		gets(buscar);
		...
		do
		{
			...
			if(!strcmpi(buscar,dat.NombreAlbum) && !feof(pf) && dat.Reliminado == 1)
			{
				system("cls");
				ShowR ();
				...
				do
				{
					gotoxy(30,11); cout << "¿Este es el Registro que Quieres Restaurar? [S] / [N]";
					respuesta=getch();
				...
				{	
					...
					system("cls");
					gotoxy(42,14); cout << "¡Registro Restaurado con exito!";
					getch();		
				}
					if(respuesta!= 'n' && respuesta!= 'N' && respuesta!= 's' && respuesta!= 'S' )
					{
		...
		{
			system("cls");
			gotoxy(38,14); cout << "¿Deseas Seguir Recuperando Registros? [S] / [N]";
			respuesta2=getch();
...
/*void Editar ()
{
	...
	{	
		...
		{
			system("cls");
			gotoxy(42,14); cout << "¿Deseas Seguir Editando? [S] / [N]";
			respuesta2=getch();
